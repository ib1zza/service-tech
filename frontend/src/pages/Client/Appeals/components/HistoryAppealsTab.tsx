...
import { useState, useEffect } from "react"; // Импорт хуков React для управления состоянием и побочными эффектами
import { AppealCompleted, appealApi } from "../../../../services/requests"; // Импорт API для работы с заявками и типа данных для завершенных заявок
...
// Вспомогательная функция для сравнения двух значений при сортировке по убыванию.
...
// Вспомогательная функция для безопасного получения значения из объекта по ключу,
...
// Функция для применения сортировки и фильтрации к массиву данных.
...
// Вспомогательная функция для форматирования даты в локальный строковый формат.
...
// Вспомогательная функция для форматирования даты и времени в отдельные строки.
...
const calculateAppealCounts = (appeals: AppealCompleted[]): AppealCounts => {
  ...
  for (const appeal of appeals) {
    ...
    // Проверка диапазона. Сравниваем дату закрытия!
...
// Компонент `HistoryAppealsTab` отображает историю завершенных заявок с возможностью фильтрации и сортировки.
export default function HistoryAppealsTab() {
  ...
  // Хук `useEffect` для загрузки истории заявок при монтировании компонента или изменении данных пользователя.
  useEffect(() => {
    const fetchAppeals = async () => {
      try {
        setLoading(true); // Установка состояния загрузки.
        const data = await appealApi.getCompletedAppeals(); // Получение завершенных заявок.
        setAppeals(data); // Обновление состояния заявок.
        ...
        setError("Не удалось загрузить историю заявок"); // Установка сообщения об ошибке.
      } finally {
  ...
  // Обработчик клика по строке таблицы, открывает диалоговое окно с деталями заявки.
  const handleRowClick = (appeal: AppealCompleted) => {
    setSelectedAppeal(appeal); // Установка выбранной заявки.
    setDialogOpen(true); // Открытие диалога.
  };
  ...
  // Обработчик сортировки таблицы по определенному полю.
  const handleSort = (property: keyof AppealCompleted) => {
    const isAsc = orderBy === property && order === "asc";
    setOrder(isAsc ? "desc" : "asc"); // Переключение направления сортировки.
    setOrderBy(property); // Установка поля сортировки.
  };

  // Применение фильтров и сортировки к списку заявок.
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      {/* Условный рендеринг: если нет завершенных заявок, отображаем сообщение. */}
      {filteredAndSortedAppeals.length === 0 ? (
        <Paper elevation={3} sx={{ p: 3, textAlign: "center" }}>
          <Typography variant="body1">Нет завершенных заявок</Typography>
        </Paper>
      ) : (
        // Если заявки есть, отображаем их в таблице.
          ...
          <Table>
            ...
            <TableBody>
              ...
              {filteredAndSortedAppeals.map((appeal) => {
                const dateStart = formatDateTime(appeal.date_start);
      ...
      {/* Диалоговое окно для отображения деталей выбранной заявки */}
      <Dialog
        open={dialogOpen}

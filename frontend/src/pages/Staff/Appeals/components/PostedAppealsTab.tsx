...
export default function PostedAppealsTab() {
    ...
    const formatDateTime = (dateString: string) => {
        ...
        return {
            ...
            time: date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) // Форматирование времени
        };
    };
                        ...
                        return (
                            <Paper key={appeal.id} elevation={3} sx={{ p: 3, mb: 3 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Typography variant="h6">Заявка №{appeal.id}</Typography>
                                    <Chip
                                        label={appeal.status.st === 'new' ? 'Новая' : appeal.status.st}
                                ...
                                <Box sx={{
                                    ...
                                    <Box>
                                        ...
                                        <Typography>{appeal.company_name_id.company_name}</Typography>
                                    </Box>
                                    <Box>
                                        <Typography variant="subtitle2">Оборудование (механизм)</Typography>
                                        <Typography>{appeal.mechanism}</Typography>
                                    </Box>
                                    <Box>
                                        <Typography variant="subtitle2">Описание проблемы</Typography>
                                        <Typography sx={{ whiteSpace: 'pre-line' }}>{appeal.problem}</Typography>
                                    </Box>
                                    <Box>
                ...
                <DialogContent>
                    <DialogContentText>
                        <Typography variant="body1" gutterBottom>
                            Заявка №{selectedAppeal?.id} от {selectedAppeal && formatDateTime(selectedAppeal.date_start).date}
                        </Typography>
                        <Typography variant="body1" gutterBottom>

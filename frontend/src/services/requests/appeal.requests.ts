...
interface CreateAppealData {
  ...
  priority: string; // Приоритет заявки.
}
...
export const appealApi = {
   ...
   * @returns Promise с данными ответа от сервера.
   */
  cancelAppeal: async (appealId: number, initiator: string) => {
    // Передаем объект с данными вторым аргументом в api.patch
    const response = await api.patch(`/appeals/${appealId}/cancel`, {
      cancel_initiator: initiator, // Название поля (cancel_initiator) должно совпадать с тем, что ожидает ваш бэкенд
    });
    return response.data;
  },
};

...
const api = axios.create({
    baseURL: 'http://localhost:5000/api', // Устанавливаем базовый URL для всех запросов.
    // Например, запрос `api.get('/users')` будет отправлен на `http://localhost:5000/api/users`.
});
